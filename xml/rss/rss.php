	...
	if($generate==1)
		...
		if($itemCount>0)
			...
			if($handle)
				...
				$object->setText(Aastra_get_label('The information you are looking for is not available at this time. Try again later.',$language));
				if($nb_softkeys) $object->addSoftkey($nb_softkeys, Aastra_get_label('Exit',$language), 'SoftKey:Exit');
				$output=$object->output();
				exit;
				}
			}
	...
	if(empty($rank))
		...
		if($count>1)
			...
			if($is_toptitle_supported) 
				{
				if($last=='1') $object->setTopTitle($Title[$index]);
				else $object->setTopTitle(sprintf($Title[$index].' (%d/%d)',$page,$last));
				}
			else 
				{
			...
			if(Aastra_is_softkeys_supported())
				...
				if($nb_softkeys==4)
					...
					if(($page!=1) or ($page!=$last))
						...
						if($nb_rss>1)
							{
							$object->addSoftkey('5', Aastra_get_label('Back',$language), $XML_SERVER.'&last_index='.($index+1));
						...
						if($nb_rss>1)
							{
							$object->addSoftkey('3', Aastra_get_label('Back',$language), $XML_SERVER.'&last_index='.($index+1));
							$object->setCancelAction($XML_SERVER.'&last_index='.($index+1));
							}
