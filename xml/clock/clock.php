...
switch($action)
	...
	case 'select':
		...
		if($is_toptitle_supported) $object->setTopTitle(sprintf(Aastra_get_label('World Clock (%s-%s)',$language),$start,$end));
		else $object->setTitle(sprintf(Aastra_get_label('World Clock (%s-%s)',$language),$start,$end));
		...
		if($nb_softkeys>0)
			...
			if($nb_softkeys==4)
				...
				if($page!=$last) $object->addSoftkey('3', Aastra_get_label('Next',$language), $XML_SERVER.'?page='.($page+1).'&user='.$user.'&action='.$action.'&rank='.$rank);
				if($action=='list') $object->addSoftkey('4', Aastra_get_label('Favorites',$language), $XML_SERVER.'?page='.$page.'&user='.$user.'&action=favorites');
		...
		if($handle)
			...
			if($found==2)
				...
				if(Aastra_is_formattedtextscreen_supported())
					...
					else $object->addLine($title,$font,'center');
				...
				if($nb_softkeys>0)
					...
					if($nb_softkeys==4)
						{
						if($action=='zoom') $object->addSoftkey('3', Aastra_get_label('Back',$language), $XML_SERVER.'?selection='.$selection.'&user='.$user.'&action=list'.'&page='.$page);
						else $object->addSoftkey('3', Aastra_get_label('Back',$language), $XML_SERVER.'?selection='.$selection.'&user='.$user.'&action=favorites');
