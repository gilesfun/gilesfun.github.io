...
if($zip=='')
	...
	# Input zip code
 	...
 	if($is_toptitle_supported) $object->setTopTitle(Aastra_get_label('US Local Weather',$language));
	else $object->setTitle(Aastra_get_label('US Local Weather',$language));
	...
	if($generate==1)
		...
		if($search[0])
			...
			if($is_toptitle_supported) $object->setTopTitle(Aastra_get_label('Information not available',$language),'red');
			else $object->setTitle(Aastra_get_label('Information not available',$language));
		...
		if($itemCount!=0)
			...
			$object=new AastraIPPhoneTextScreen();
			if($is_toptitle_supported) $object->setTopTitle(Aastra_get_label('Error',$language),'red');
			else $object->setTitle(Aastra_get_label('Error',$language));
	...
	if(empty($index))
		...
		if($nb_softkeys<=6)
			...
			else $object->setTitle($array_rss[99]['title']);
		...
		if($header['model']!='Aastra6739i') 
			...
			if($is_toptitle_supported) 
				{
				$object->setTopTitle($array_rss[99]['title']);
				$object->setTitle($array_rss[$index]['title']);
				}
		...
		if($nb_softkeys) 
			...
			if($nb_softkeys==4)
				{
				if($index!=0) $object->addSoftkey('1', Aastra_get_label('Previous',$language), $XML_SERVER.'&zip='.$zip.'&index='.$index);
				$object->addSoftkey('3', Aastra_get_label('Back',$language), $XML_SERVER.'&zip='.$zip);
				if($index!=(count($array_rss)-2))$object->addSoftkey('2', Aastra_get_label('Next',$language), $XML_SERVER.'&zip='.$zip.'&index='.($index+2));
